...
public static class ClassConstruction
{
    ...
    public static (ClassDeclarationSyntax classDeclarationSyntax ,ConstructorDeclarationSyntax constructor) CreateClassInheritingFromEntityGeneric<T>(
        this TypeContext<T> typeContext,
        string className,
        string baseClassName) where T : TypeDeclarationSyntax
    {
        SyntaxToken entityName = typeContext.Node.Identifier;
        
        GenericNameSyntax super = SyntaxFactory.GenericName(SyntaxFactory.Identifier(baseClassName))
            .WithTypeArgumentList(SyntaxFactory.TypeArgumentList(
                SyntaxFactory.SingletonSeparatedList<TypeSyntax>(
                    SyntaxFactory.IdentifierName(entityName))));
        
        SyntaxTokenList publicSealedPartial = SyntaxFactory.TokenList(
        ...
        ) where T : TypeDeclarationSyntax
    {
        ...
        GenericNameSyntax super = SyntaxFactory.GenericName(SyntaxFactory.Identifier(@interface))
            .WithTypeArgumentList(SyntaxFactory.TypeArgumentList(
                SyntaxFactory.SingletonSeparatedList<TypeSyntax>(
                    SyntaxFactory.IdentifierName(entityName))));
    ...
    public static PropertyDeclarationSyntax PropertyWithPublicGetter(SyntaxKind typeKind, string propertyName, string? literalValue = null)
    {
    ...
    {
        return NewObjectWithCtor(objectname, ctorArgs.Select(e => SyntaxFactory.Argument(SyntaxFactory.IdentifierName(e))).ToList());
    }
