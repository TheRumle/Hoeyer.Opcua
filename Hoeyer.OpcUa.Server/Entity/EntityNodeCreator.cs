...
internal sealed class EntityNodeCreator<TEntity> : IEntityNodeCreator
{
    ...
    public NodeState CreateEntityOpcUaNode(NodeState root, ushort namespaceIndex)
    {
        ...
        foreach (var publicProperty in readableProperties) AddVariable(publicProperty, entityNode, namespaceIndex);
    ...
    {
        var entityNode = new BaseObjectState(root)
        {
    ...
    private static void AddVariable(PropertyInfo property, NodeState entity, ushort dynamicNamespaceIndex)
    {
        ...
        var (dataTypeId, _) = SupportedVariableTypeCallbackHandler.ToDataTypeId(property);
        var a = entity.AddProperty<int>(property.Name, dataTypeId, ValueRanks.Scalar);
