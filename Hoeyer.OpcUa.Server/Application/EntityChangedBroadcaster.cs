...
public sealed class EntityChangedBroadcaster<T>(IEntityTranslator<T> translator)
    : IEntityChangedBroadcaster<T>
{
    private IEntityNode? Node { get; set; }

    public ISubscriptionManager<IEntityNode> NodeSubscriptionManager { get; } =
        new SubscriptionManager<IEntityNode>(new ChannelSubscriptionFactory<IEntityNode>());

    public ISubscriptionManager<T> EntitySubscriptionManager { get; } =
        new SubscriptionManager<T>(new ChannelSubscriptionFactory<T>());
    ...
    private ServiceResult OnPropertyWritten(ISystemContext context, NodeState node, NumericRange indexrange,
