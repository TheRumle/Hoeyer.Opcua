...
internal record struct MemberTypeSupport(bool IsSupported, Location Location, params IEnumerable<string> TypesWithError)
{
    ...
    public static MemberTypeSupport Failure(MemberDeclarationSyntax syntax, List<ITypeSymbol> errors) => new MemberTypeSupport(false, syntax.GetLocation(), string.Join(", ", errors.Select(t => t.ToString())));
}