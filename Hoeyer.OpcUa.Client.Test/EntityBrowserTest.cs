...
public abstract class EntityBrowserTest(SimulationTestServiceList<IEntityBrowser> simulationFixtures)
{
    [Test]
        ...
        CancellationToken token)
    {
        ...
        await simulationFixtures.AssertAllServices(CanGetAnyValuesFromEntity);
    }

    [Test]
    public async Task EntityBrowser_BrowsedEntity_DoesNotHaveNullValues(CancellationToken token)
    {
        await simulationFixtures.AssertAllServices(async browser =>
        {
            var entity = await browser.BrowseEntityNode(token);
